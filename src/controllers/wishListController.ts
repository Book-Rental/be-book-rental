...
export const getUserWishlists = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    try {
        ...
        const wishlists = await getWishlistsByUserService(userId, req.query);

        successResponse(res, wishlists, "Wishlists fetched successfully.", 200);
    } catch (error) {
        console.error("Get Wishlists Error:", error);

        errorResponse(res, error instanceof Error ? error.message : "Failed to fetch wishlists.");
    }
};
...
export const createWishlistGroup = async (
    ...
    try {
        ...
        const wishlist = await createWishlistGroupService(userId, name, description);
...
export const removeBookFromWishlist = async (
    ...
    try {
        ...
        const wishlist = await removeBookFromWishlistService(wishlistId, bookId);

        successResponse(res, wishlist, "Book removed from wishlist successfully.", 200);
    } catch (error) {
...
export const getUserWishlistNames = async (
    ...
    try {
        ...
        successResponse(res, wishlists, "Wishlist names fetched successfully.", 200);
    } catch (error) {
        console.error("Get Wishlist Names Error:", error);

        errorResponse(
            res,
            error instanceof Error ? error.message : "Failed to fetch wishlist names."
        );
    }
};
