...
export const getWishlistsByUserService = async (userId: string, query: any) => {
    try {
        const { wishListID, page = 1, limit = 5 } = query;
        ...
        const pipeline = buildWishlistBooksAggregationPipeline(filter, pageNumber, limitNumber);
        ...
        const hasMore = (pageNumber - 1) * limitNumber + books.length < totalRecords;
...
export const addBookToWishlistService = async (
    ...
    try {
        ...
        throw error instanceof Error ? error : new Error("Failed to add book to wishlist.");
    }
};
...
export const removeBookFromWishlistService = async (
    ...
    try {
        ...
        const bookExists = wishlist.items.some((item) => item.bookId.toString() === bId.toString());
        ...
        // Remove the book
        wishlist.items = wishlist.items.filter((item) => item.bookId.toString() !== bId.toString());
        ...
        throw error instanceof Error ? error : new Error("Failed to remove book from wishlist.");
    }
};

export const getUserWishlistNamesService = async (userId: string) => {
    try {
        if (!mongoose.Types.ObjectId.isValid(userId)) {
            throw new Error("Invalid user id.");
        }
        ...
        throw error instanceof Error ? error : new Error("Failed to fetch wishlist names.");
    }
};

export const getUserWishlistsWithBooksService = async (userId: string) => {
    try {
        if (!mongoose.Types.ObjectId.isValid(userId)) {
            throw new Error("Invalid user id.");
        }
        ...
        throw error instanceof Error ? error : new Error("Failed to fetch wishlists.");
    }
};
